...
var debug = require('debug')('mongoose-flex-rest:acl');
...
exports.createRules = function(Schema, globals){
  ...
  function createMap(tree, acl, context){
    //Ignore virtual getter for _id
    tree = und.omit(und.isArray(tree) ? tree[0] : tree, ['id']);
    und.each(tree, function(property, path){
      if (path === '_id' || path === '__v'){
        allPaths.push(context ? context + '.' + path : path);
        return applyDefaults(path);
      }
      ...
      if (und.isObject(property.acl)){ //plain path with defined acl
        ...
        if (property[0] instanceof mongoose.Schema){
          //substitute reference to Schema with actual schema and iterate further
          property[0] = property[0].tree;
          createMap(property[0], acl, context ? context + '.' + path : path);
...
exports.validateRead = function(query, acl, model){
  ...
  paths.forEach(function(path){
    if (!validate('read', modelAcl, uacl, path)){
      debug('Read validation failed for path %s with level %s', path, uacl);
      allow = false;
    }
  });
  return allow;
...
exports.validateUpdate = function(cmd, acl, model){
  ...
  paths.forEach(function(path){
    if (!validate('update', model.schema.acl, level, path)){
      debug('Update validation failed for path %s with level %s', path, level);
      allow = false;
    }
  });
  return allow;
...
exports.validatePop = function(popObj, acl, model){
  ...
  var path = model.schema.tree;
  popObj.path.split('.').forEach(function(prop){
    if (und.isArray(path[prop])){
      path[prop] = path[prop][0];
      path = path[prop];
    }else{
      path[prop] = path[prop] || {};
      path = path[prop];
...
exports.optimisticPopulate = function(initialObject, acl, model){
  //get child model by ref
  var popObj = und.extend({}, initialObject);
  var path = model.schema.tree;
  popObj.path.split('.').forEach(function(prop){
    if (und.isArray(path[prop])){
      path[prop] = path[prop][0];
  ...
  var populatedSchema = model.prototype.model(ref);
  if (!popObj.select){
    popObj.select = exports.getAllowed(acl, populatedSchema);
  }else{
    popObj.select = exports.optimisticSelect(popObj.select, acl, populatedSchema);
  }
  return popObj;
};
  ...
  paths.forEach(function(path){
    if (!/^-/.test(path)){ //ignore excluded paths
      ...
      if (!validate('read', model.schema.acl, acl.read || 0, path)){
        debug('Select validation failed for path %s with level %s', path, acl.read || 0);
        ok = false;
      }
    }
  });
...
exports.optimisticSelect = function(str, acl, model){
  var paths = [];
  if (str.length === 0 ){
    paths = model.schema.acl.all;
  }else{
  ...
  var valid = paths.map(function(path){
    if (!/^-/.test(path)){ //ignore excluded paths
      var normalised = path.replace(/^\+/, ''); //validate included paths
      if (!validate('read', model.schema.acl, acl.read || 0, normalised)){
        return null;
      }else{
        return path;
      }
