  ...
  function createMap(tree, acl, context){
    ...
    und.each(tree, function(property, path){
      ...
      if (und.isObject(property.acl)){
        ...
        //TODO test this
        if (und.isFunction(property.type)){
          ['create', 'read', 'update', 'delete'].forEach(function(key){
            if (!und.isObject(acl['$' + globals[key]])) acl['$' + globals[key]] = {};
            if (!und.isArray(acl['$' + globals[key]][key])) acl['$' + globals[key]][key] = [];
            acl['$' + globals[key] || 0][key].push(context ? context + '.' + path : path);
          });
        }else{
          createMap(tree[path], acl, context ? context + '.' + path : path);
        }
