...
exports.validateCreate = function(user, model){
  return user.acl.create >= model.schema.acl.create;
};

exports.validateDelete = function(user, model){
  return user.acl.delete >= model.schema.acl.delete;
};

/**
 *
 * @param query - query to validate
 * @param user - user with access rights defined
 * @param model - model to validate against
 */
exports.validateRead = function(query, user, model){
  if (!model.schema.acl) throw new Error('Model has no acl'); //No acl specified for this model.
  var acl = model.schema.acl.paths;
  var uacl = user.acl.read;
  var paths = [];
  parseQuery(paths, query, '');
  var allow = true;
  paths.forEach(function(path){
    if (!validate('read', acl, uacl, path)){
      allow = false;
    }
  });
  return allow;
...
exports.validateUpdate = function(cmd, user, model){
  ...
  var allow = true;
  paths.forEach(function(path){
    if (!validate('update', model.schema.acl, user.acl.update, path)){
      allow = false;
    }
  });
  return allow;
...
function parseQuery(arr, query, context, pathToSkip){
  if (pathToSkip){
    query = query[pathToSkip];
  }
  und.each(query, function(nested, path){
    if (und.isObject(nested) && !und.isRegExp(nested)){
      ...
      if (iterate){
        parseQuery(arr, query[path], context ? context + '.' + path : path, skip);
      }else{
...
function validate(action, model, user, path){
  try{
    if (model['$' + user][action].indexOf(path) != -1){
      return true;
    }else{
      if (user > 0){
    ...
    return Object.keys(model).length - 1 < user;
  }
}
