...
module.exports = function(globals, middleware){
   ...
   * @param accessLevel - not used at the moment
   * @param args - custom arguments to middleware. always: [data, acccessLevel]
   * @param callback - if function returns saved document
   * @returns {model} - mongoose document. Returned when no callback passed
   */
  function create(data, accessLevel, args, callback){
    if (arguments.length > 1){
   ...
   * @param args - custom arguments for middleware. always: [query, command (not parsed)]
   * @param callback - required
   */
    //TODO handle __v control errors?
  function update(query, command, args, callback){
    if (arguments.length > 1){
      if (und.isFunction(query)){
        callback = query;
        args = [];
  ...
  function del(query, args, callback){
    var model = this;
    if (und.isFunction(args)){
      callback = args;
      args = [];
    }
    if (!callback) throw new Error('Callback required');
    query.access = query.access || {};
    query.access.delete = query.access.delete || 0;
    query.access.read = query.access.read || 0;
    ...
    middleware.run('all', model, args, function(err){
      ...
      middleware.run('delete', model, args, function(err){
        if (err) return callback(err);
        if (!acl.validateRead(query.find, query.access.read, model)){
          return callback(new Error('Access denied: read'));
        }
        if (!acl.validateDelete(query.access, model)){
