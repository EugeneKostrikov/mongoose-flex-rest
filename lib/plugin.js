...
var plugin = function(Schema, globals){
  ...
  globals.acl = globals.acl || {create: 0, delete: 0};
  ...
  function head(query, options, callback){
    ...
    if (!acl.validateRead(query, options.user, this)){
  ...
  function read(query, options, callback){
    ...
    //if (!options.user) return callback(new Error({status: 403, message: 'Access denied'}));
    ...
    query = parse.query(query);
    if (!acl.validateRead(query, options.user, model)){
      throw new Error('Access denied');
    }
  ...
  function create(data, options, callback){
    ...
    var model = this;
    if (!acl.validateCreate(options.user, model)){
      throw new Error('Access denied');
    }
  ...
  function update(query, command, options, callback){
    ...
    if (!acl.validateRead(q, options.user, model)){
      throw new Error('Access denied');
    }
    ...
    model.find(q, options.select, function(err, docs){
      ...
      _.each(docs, function(document){
        ...
        if (!acl.validateUpdate(command, options.user, model)){
          throw new Error('Access denied');
        }
  ...
  function del(id, user, callback){
    var model = this;
    if (!callback) throw new Error('Callback required');
    if (!acl.validateDelete(user, model)){
      throw new Error('Access denied');
    }
